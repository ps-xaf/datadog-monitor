resource "datadog_monitor" "network_alert" {
  ...
  query = coalesce(
    lookup(each.value, "query", ""), templatefile(
      fileexists("${path.root}/${var.path_templates}/query/network_${lookup(each.value, "query_tpl", "default")}.tpl")
      ? "${path.root}/${var.path_templates}/query/network_${lookup(each.value, "query_tpl", "default")}.tpl"
      : "${path.module}/../templates/query/network_${lookup(each.value, "query_tpl", "default")}.tpl",
      {
        name      = each.key,
  ...
  message = templatefile(
    fileexists("${path.root}/${var.path_templates}/message/network_${lookup(each.value, "msg_tpl", "default")}.tpl")
    ? "${path.root}/${var.path_templates}/message/network_${lookup(each.value, "msg_tpl", "default")}.tpl"
    : "${path.module}/../templates/message/network_${lookup(each.value, "msg_tpl", "default")}.tpl",
    {
      name      = each.key,
